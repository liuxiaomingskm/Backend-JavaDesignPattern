...
class HotDrinkMachine
{
    ...
    public HotDrinkMachine() throws Exception
    {
        ...
        for (AvailableDrink drink : AvailableDrink.values()) // the values method return all the values represented in enum
        {
            String s = drink.toString();
            String factoryName = "" + Character.toUpperCase(s.charAt(0)) + s.substring(1).toLowerCase();

            /**
             * "?" is a wildcard mean all type is allowed, class.forName() returns the Class object associated with the class or interface with the given string name
             */
            Class<?> factory = Class.forName("com.example.factories." + factoryName + "Factory");
            /**
             * getDeclaredConstructor() returns a Constructor object that reflects the specified constructor of the class or interace
        ...
        /**
         * return all subclass object of IHotDrinkFactory
         */
        Set<Class<? extends IHotDrinkFactory>> types =
                new Reflections("com.example.factories")
                        .getSubTypesOf(IHotDrinkFactory.class);
        for (Class<? extends IHotDrinkFactory> type : types)
        {
            namedFactories.add(new Pair<>(
                    /**
                     * getSimpleName() returns class name, not full path name, ie. mother rather than com.concer.mother
                     */
